...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    ...
    c.innerHTML =
      ...
      // LOCAL EXTENSION (not upstream): `detail` is injected verbatim so a
      // section can carry rich content the engine has no field for — skill
      // meters, certificate cards, project cards, contact links. It is built
      // on the server from this site's own database and escaped there; never
      // pass unescaped visitor input through it.
      (s.detail ? `<div class="sw-copy__detail">${s.detail}</div>` : '') +
...
function injectCSS() {
  ...
  .sw-copy__detail{margin-top:20px;pointer-events:auto;}
...
// LOCAL EXTENSION: exported so the Next app can import it as a module.
